...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void) {
            ...
            int enter = int(gl_InstanceID/10); 

            gl_Position = P_matrix * V_matrix * M_matrix * (position + gl_InstanceID%10 * vec4(1, 0, 0, 0) + enter * vec4(0, 1, 0, 0));
...
def render(time):
    ...
    '''for i in range(10):
        ...
        M_matrix = glm.translate(M_matrix, glm.vec3(0.0, -10.0, 0.0))
'''
    M_matrix = glm.translate(M_matrix, glm.vec3(1.0, 0.0, 0.0))
    glUniformMatrix4fv(M_location, 1, GL_FALSE, glm.value_ptr(M_matrix))
...
def update_viewport(window, width, height):
    ...
    aspect = width / height
    P_matrix = glm.perspective(glm.radians(100.0), aspect, 0.1, 1000.0)
