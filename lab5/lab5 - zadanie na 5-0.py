...
def render(time):
    ...
    for i in range(0,N):
        for j in range(0,N):
            ...
            if(j>=0.5*N):
                vektory[i][j][1]=vektory[i][j][1]*(-1)

                vektory[i][j][0]=vektory[i][j][0]*(-1)
                vektory[i][j][2]=vektory[i][j][2]*(-1)
    ...
    for i in range(0,N):
        for j in range(0,N):
            ...
            glColor3ub(random.randint(0,255), random.randint(0,255), random.randint(0,255))
            glNormal(vektory[i][j][0],vektory[i][j][1],vektory[i][j][2])
            glVertex3f(tab[i][j][0]+vektory[i][j][0], tab[i][j][1]+vektory[i][j][1], tab[i][j][2]+vektory[i][j][2])
            glEnd()
